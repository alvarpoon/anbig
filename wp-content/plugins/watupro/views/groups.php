<div class="wrap">
	...
	<div id="watuproUSerGroups" style="display:<?php echo empty($use_wp_roles)?'block':'none;'?>">
		...
		<form method="post">
		<p><input type="checkbox" name="select_group_on_signup" value="1" <?php if(get_option('watupro_select_group_on_signup')==1) echo 'checked'?>> <?php _e('Allow users to select user group on the regisration page', 'watupro')?>
			<input type="submit" name="signup_options" value="<?php _e('Save', 'watupro');?>"
		</p>
		</form>
	</div>
</div>
